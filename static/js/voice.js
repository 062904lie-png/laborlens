...
(function () {
  ...
  function speak(text, language, anchor) {
    ...
    // Keep the requested Filipino voice across all answer languages.
    speech.lang = 'fil-PH';
    const voices = window.speechSynthesis.getVoices?.() || [];
    const aliases = ['fil','tl'];
    ...
    if (!matchingVoice) {
      notify('A Filipino voice is not available in this browser. Reading is paused; try again once a Filipino voice is available.');
      return;
    }
    ...
    try { window.speechSynthesis.speak(speech); } catch (_) { stopSpeaking(); }
  }
