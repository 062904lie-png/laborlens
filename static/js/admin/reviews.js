...
window.AdminReviews=(()=>{
 ...
 const sourceLabels=new Set(['High confidence match','Source match','Partial source match','Insufficient legal source','Conflicting sources','Temporary response issue','Not Required']);
 const unresolvedLabels=new Set(['Insufficient legal source','Conflicting sources','Temporary response issue']);
 const noRetrievalIntents=new Set(['greeting','small_talk','identity','identity_question','capability_question','gratitude','thanks','farewell','goodbye','acknowledgement']);
 ...
 function sourceMatchLabel(row={}){
  ...
  if(level==='not required'||noRetrievalIntents.has(String(row.intent||'').trim().toLowerCase()))return 'Not Required';
 ...
 function sourceMatchBadge(row){
  ...
  const color=label==='High confidence match'?'green':label==='Source match'?'blue':label==='Partial source match'?'amber':label==='Conflicting sources'?'amber':label==='Not Required'?'gray':'red';
