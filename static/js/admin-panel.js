...
(() => {
  ...
  function enhanceCards() {
    const keys={Users:'users',Chats:'queries',Documents:'knowledge',Review:'check',Queries:'queries',Sessions:'queries',Feedback:'check',Speed:'performance',Slow:'performance',Total:'analytics','♙':'users','☏':'queries','▤':'knowledge','▥':'knowledge','?':'queries','◷':'performance','✓':'check','⚑':'check'};
  ...
  function setup() {
    // Keep the upload form short while preserving every existing field and handler.
    const uploadBody=document.querySelector('#tab-knowledge .two-col .panel-body');
    const firstExtra=document.getElementById('kb-document-type');
    const uploadZone=document.querySelector('#tab-knowledge .upload-zone');
    if(uploadBody && firstExtra && uploadZone){
      const details=document.createElement('details');details.className='admin-extra-fields';
      const summary=document.createElement('summary');summary.textContent='More document details (optional)';details.append(summary);
      let field=firstExtra.closest('.form-row');uploadBody.insertBefore(details,field);
      while(field && field!==uploadZone){const next=field.nextElementSibling;details.append(field);field=next;}
    }
    const docTable=document.getElementById('docs-tbody')?.closest('table');
    if(docTable)docTable.classList.add('admin-doc-table');
    document.querySelectorAll('#dashboard .page-hdr-left h1').forEach(title=>{
      const words=title.textContent.split(' ');if(words.length>1){title.innerHTML=escape(words.shift())+' <span class="admin-heading-accent">'+escape(words.join(' '))+'</span>';}
    });
